# VoiceInk — Dashboard redesign + app-wide theming
...
New `ThemeManager: ObservableObject`, persisted via `@AppStorage`, injected at root in `VoiceInk.swift`.
...
1. `xcodebuild -scheme VoiceInk CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO build` → BUILD SUCCEEDED.
