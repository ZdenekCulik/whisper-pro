...
enum AppSkin: String, CaseIterable, Identifiable {
    ...
    // Light/Dark use the app's native semantic colors — no overrides.
...
final class ThemeManager: ObservableObject {
    ...
    init() {
        ...
        skin = savedSkin.flatMap(AppSkin.init(rawValue:)) ?? .light

        let savedFont = UserDefaults.standard.string(forKey: fontKey)
        fontChoice = savedFont.flatMap(AppFontChoice.init(rawValue:)) ?? .system
    }
    ...
    /// Background to paint behind content. nil for Light/Dark (keep current window background).
    var resolvedBackground: Color? { skin.background }

    /// Accent/tint. nil for Light/Dark so `Color.accentColor` stays system blue.
    var resolvedAccent: Color? { skin.accent }
