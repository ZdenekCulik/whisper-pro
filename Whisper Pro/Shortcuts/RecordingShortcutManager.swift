...
class RecordingShortcutManager: ObservableObject {
    ...
    private var lastAccessibilityTrusted = AXIsProcessTrusted()
    ...
    init(engine: WhisperProEngine, recorderUIManager: RecorderUIManager) {
        ShortcutMigration.migrateLegacyShortcutsIfNeeded()
        ShortcutStore.seedShortcut(.command, for: .primaryRecording, replacingCleared: true)
        ...
        // macOS posts this when Accessibility permission changes. If the user grants it
        // while the app is running, the global event tap was installed as a local-only
        // fallback — reinstall it so global shortcuts (outside the app) start working
        ...
        accessibilityObserver = DistributedNotificationCenter.default().addObserver(
            ...
            Task { @MainActor in
                ...
                guard trusted != self.lastAccessibilityTrusted else { return }
