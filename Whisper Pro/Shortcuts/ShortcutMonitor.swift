...
final class ShortcutMonitor {
    ...
    private let logger = Logger(subsystem: "com.prakashjoshipax.whisperpro", category: "ShortcutMonitor")
    ...
    private func installEventTap() -> Bool {
        ...
        guard let eventTap = CGEvent.tapCreate(
            ...
            return installLocalEventMonitorFallback()
        }
    ...
    private func installLocalEventMonitorFallback() -> Bool {
        let monitor = NSEvent.addLocalMonitorForEvents(matching: [.keyDown, .keyUp, .flagsChanged]) { [weak self] event in
            guard let self,
                  let eventKind = EventKind(event.type) else {
                return event
            }

            let shouldSuppress = self.handleEvent(
                kind: eventKind,
        ...
        localEventMonitor = monitor
        logger.info("Installed local shortcut monitor fallback")
        return true
    }
