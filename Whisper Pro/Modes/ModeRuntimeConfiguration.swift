...
enum ModeRuntimeResolver {
    ...
    private static func resolvedModel(
        ...
        // No mode-specific model: fall back to the model the user picked in AI Models
        // (CurrentTranscriptionModel) so their choice is honored even without modes.
        if let current = transcriptionModelManager.currentTranscriptionModel,
           transcriptionModelManager.usableModels.contains(where: { $0.name == current.name }) {
            return current
        }
