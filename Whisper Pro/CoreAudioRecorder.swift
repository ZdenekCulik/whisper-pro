...
final class CoreAudioRecorder: @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: "com.prakashjoshipax.whisperpro", category: "CoreAudioRecorder")
    ...
    // True when the live unit is VoiceProcessingIO. VPIO holds the system output
    // device (echo reference) for as long as it stays initialized, which keeps music
    // ducked. So when VPIO is active we fully release the unit on stop instead of
    // keeping it warm; HAL input doesn't touch output and can stay prepared.
    ...
    // Channels delivered to our callback (= callback stream format). For plain HAL this
    // matches the device; VoiceProcessingIO delivers mono, so the callback/meters/file
    // conversion all key off this instead of the raw device channel count.
    ...
    private func setupAudioPipeline(deviceID: AudioDeviceID, voiceProcessing: Bool) throws {
        try createAudioUnit(voiceProcessing: voiceProcessing)
        try setInputDevice(deviceID)
        try configureFormats(voiceProcessing: voiceProcessing)
    ...
    func stopRecording() {
        ...
        // VPIO keeps the system output device captured (and music ducked) until the
        // unit is uninitialized — stopping isn't enough. Fully release it so audio
        // returns to normal; it's re-prepared on the next recording.
    ...
    func switchDevice(to newDeviceID: AudioDeviceID) throws {
        ...
        // Step 5: Configure callback format for new device. This live-switch path uses
        // the plain HAL unit, so the callback receives the device's own channel count.
        captureChannels = newDeviceFormat.mChannelsPerFrame
    ...
    private func createAudioUnit(voiceProcessing: Bool) throws {
        ...
        guard let component = AudioComponentFindNext(nil, &desc) else {
            logger.error("AudioUnit not found - output component unavailable")
        ...
        self.isVoiceProcessing = voiceProcessing
        ...
        if voiceProcessing {
            // VoiceProcessingIO requires the output bus (element 0) enabled — it's the
            // echo reference. (Disabling it makes initialization fail with -10875.)
            // Make sure echo cancellation isn't bypassed and AGC is on.
            var bypass: UInt32 = 0
            ...
            // Plain HAL capture: disable the unused output bus (element 0).
    ...
    private func configureFormats(voiceProcessing: Bool) throws {
        guard let audioUnit = audioUnit else {
            throw CoreAudioRecorderError.audioUnitNotInitialized
        }
        ...
        // VoiceProcessingIO delivers mono; plain HAL keeps the device's channel count.
        captureChannels = voiceProcessing ? 1 : deviceFormat.mChannelsPerFrame
        ...
        var callbackFormat = AudioStreamBasicDescription(
            ...
            mFormatID: kAudioFormatLinearPCM,
            mFormatFlags: kAudioFormatFlagIsFloat | kAudioFormatFlagIsPacked,
            mBytesPerPacket: UInt32(MemoryLayout<Float32>.size) * captureChannels,
            mFramesPerPacket: 1,
            mBytesPerFrame: UInt32(MemoryLayout<Float32>.size) * captureChannels,
        ...
        // VPIO's enabled output bus (element 0) needs a matching format on its input
        // scope, otherwise the unit fails to initialize with -10875.
        if voiceProcessing {
            status = AudioUnitSetProperty(
                audioUnit,
                kAudioUnitProperty_StreamFormat,
            ...
            if status != noErr {
                logger.error("Failed to set VPIO output-bus format: \(status, privacy: .public)")
                throw CoreAudioRecorderError.failedToSetFormat(status: status)
            }
        }
        ...
        let maxFrames: UInt32 = 4096
        let bufferSamples = maxFrames * captureChannels
        renderBuffer = UnsafeMutablePointer<Float32>.allocate(capacity: Int(bufferSamples))
        renderBufferSize = bufferSamples
    ...
    private func handleInputBuffer(
        ...
        let channelCount = captureChannels
    ...
    private func calculateMeters(from bufferList: inout AudioBufferList, frameCount: UInt32) {
        ...
        let channelCount = Int(captureChannels)
    ...
    private func convertAndWriteToFile(inputBuffer: inout AudioBufferList, frameCount: UInt32) {
        ...
        let inputChannels = captureChannels
