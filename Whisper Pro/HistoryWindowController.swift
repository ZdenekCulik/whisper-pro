...
class HistoryWindowController: NSObject, NSWindowDelegate {
    ...
    private let windowIdentifier = NSUserInterfaceItemIdentifier("com.prakashjoshipax.whisperpro.historyWindow")
    private let windowAutosaveName = NSWindow.FrameAutosaveName("WhisperProHistoryWindowFrame")

    private override init() {
        super.init()
    }

    func showHistoryWindow(modelContainer: ModelContainer, engine: WhisperProEngine) {
    ...
    private func createHistoryWindow(modelContainer: ModelContainer, engine: WhisperProEngine) -> NSWindow {
