...
class DictionaryQuickAddPanel: NSPanel {
    ...
    private static func centeredOrigin(for size: NSSize) -> NSPoint {
        guard let screen = NSScreen.main ?? NSScreen.screens.first else { return NSPoint(x: 0, y: 0) }
...
struct DictionaryQuickAddView: View {
    ...
    private var vocabularyInput: some View {
        HStack(spacing: 11) {
            ...
            TextField("", text: $wordInput, prompt: Text("e.g. Prakash, Whisper Pro").foregroundColor(.secondary))
        ...
        VStack(spacing: 8) {
            ...
            HStack(spacing: 10) {
                Text("With")
                    ...
                    .foregroundStyle(.secondary)
                    .frame(width: 56, alignment: .trailing)
                TextField("", text: $replacementInput, prompt: Text("e.g. support@trywhisperpro.com").foregroundColor(.secondary))
                    .textFieldStyle(.roundedBorder)
                    .font(.system(size: 14))
