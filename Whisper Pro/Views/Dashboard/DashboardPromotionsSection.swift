...
struct DashboardPromotionsSection: View {
            ...
            HStack(alignment: .top, spacing: 18) {
                if shouldShowUpgradePromotion {
                    DashboardPromotionCard(
                        badge: "30% OFF",
                        title: "Unlock Whisper Pro For Less",
                        message: "Share Whisper Pro on your socials, and instantly unlock a 30% discount on Whisper Pro.",
                ...
                if shouldShowAffiliatePromotion {
                    DashboardPromotionCard(
                        badge: "AFFILIATE 30%",
                        title: "Earn With The Whisper Pro Affiliate Program",
                        message: "Share Whisper Pro with friends or your audience and receive 30% on every referral that upgrades.",
    ...
    private func openSocialShare() {
        if let url = URL(string: "https://trywhisperpro.com/social-share") {
            NSWorkspace.shared.open(url)
        }
    }
    
    private func openAffiliateProgram() {
        if let url = URL(string: "https://trywhisperpro.com/affiliate") {
            NSWorkspace.shared.open(url)
        }
    }
